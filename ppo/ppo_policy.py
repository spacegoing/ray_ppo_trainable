...
class PPOPolicy(PGPolicy):
  ...
  def __init__(
    ...
    '''kwargs: ['discount_factor', 'reward_normalization', 'action_scaling',
    'action_bound_method', 'lr_scheduler', 'action_space']
    '''
  ...
  def learn(  # type: ignore
    ...
    # from stablebaseline
    approx_kl_divs, explained_vars, clip_fractions = [], [], []
    ...
    for step in range(repeat):
      ...
      for minibatch in batch.split(batch_size, merge_last=True):
        ...
        with torch.no_grad():
          # approx KL
          log_ratio = dist.log_prob(minibatch.act) - minibatch.logp_old
          approx_kl_div = torch.mean((torch.exp(log_ratio) - 1) - log_ratio)
          approx_kl_divs.append(approx_kl_div.item())

          # explained var
          explained_vars.append(
              self.explained_variance(value.flatten(),
                                      minibatch.returns.flatten()).item())
          # clip fraction
          clip_fractions.append(
              torch.mean((torch.abs(ratio - 1)
                          > self._eps_clip).float()).item())
    return {
        "loss": losses,
        "loss/clip": clip_losses,
        "loss/vf": vf_losses,
        "loss/ent": ent_losses,
        "loss/approx_kl": approx_kl_divs,
        "loss/explained_var": explained_vars,
        "loss/clip_fraction": clip_fractions
    }

  def explained_variance(self, y_pred, y_true):
    assert y_true.ndim == 1 and y_pred.ndim == 1
    var_y = torch.var(y_true)
    return np.nan if var_y == 0 else 1 - torch.var(y_true - y_pred) / var_y
